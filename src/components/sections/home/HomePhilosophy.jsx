        ...
        <div className="grid gap-8 md:grid-cols-3">
          {homeData.philosophy.map((item, index) => (
            <Card
              key={index}
              luxury={true}
              className="group relative flex flex-col items-center text-center p-8 overflow-hidden"
            >
              <div className="mb-6 p-4 bg-gray-50 rounded-full transition-all duration-500 group-hover:bg-white group-hover:shadow-sm">
                {Icons[item.icon]}
              </div>
              <h3 className="font-heading text-2xl font-medium text-primary transition-colors duration-500 group-hover:text-primary/90 mb-4">
                {item.title}
              </h3>
